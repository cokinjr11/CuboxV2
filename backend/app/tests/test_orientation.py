...
def test_accepts_position_1_base_width_thickness():
    assert is_valid_orientation(WIDTH, HEIGHT, THICKNESS, dx=1200, dy=100, dz=2000) is True


def test_accepts_position_1_rotated_in_plane():
    assert is_valid_orientation(WIDTH, HEIGHT, THICKNESS, dx=100, dy=1200, dz=2000) is True


def test_accepts_position_2_base_height_thickness():
    assert is_valid_orientation(WIDTH, HEIGHT, THICKNESS, dx=2000, dy=100, dz=1200) is True


def test_accepts_position_2_rotated_in_plane():
    assert is_valid_orientation(WIDTH, HEIGHT, THICKNESS, dx=100, dy=2000, dz=1200) is True


def test_rejects_lying_flat_on_glass_face():
    """PROHIBIDO: base = Width x Height, vertical = Thickness."""
    assert is_valid_orientation(WIDTH, HEIGHT, THICKNESS, dx=1200, dy=2000, dz=100) is False


def test_rejects_lying_flat_on_glass_face_rotated():
    assert is_valid_orientation(WIDTH, HEIGHT, THICKNESS, dx=2000, dy=1200, dz=100) is False
...
def test_forbidden_orientation_never_valid_regardless_of_order(dx, dy, dz):
    assert is_valid_orientation(WIDTH, HEIGHT, THICKNESS, dx, dy, dz) is False
...
def test_square_like_window_never_lies_on_glass():
    ...
    w = h = 1000
    t = 50
    assert is_valid_orientation(w, h, t, dx=1000, dy=1000, dz=50) is False
    assert is_valid_orientation(w, h, t, dx=1000, dy=50, dz=1000) is True
